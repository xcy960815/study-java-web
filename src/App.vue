...
const getUserList = async () => {
  ...
  const result = await users.getUserList<{
    data: User[]
    total: number
  }>({ pageSize, pageNum, ...queryFormData })
  if (result.code === 200) {
...
const handleClickEditUser = (row: User) => {
...
const handleClickAddOrEditConfirm = async () => {
  const valid = await addOrEditUserFormRef.value?.validate().then(() => true).catch(() => false)

  if (!valid) return
  let result
  if (addOrEditUserDialogTitle.value === "新增用户") {
    result = await users.insertUser<boolean>(addOrEditUserFormData)
  } else {
    result = await users.updateUser<boolean>(addOrEditUserFormData)
  }
...
const handleClickDeleteUser = (row: User) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const result = await users.deleteUser<boolean>({ userId: row.userId })
