<template>
  <div class="flex items-center cursor-pointer" @click="copyToClipboard()">
    <copy v-show="buttonnStatus === 'copy'" :theme="buttonnConfig.theme" :size="buttonnConfig.size"
      :fill="buttonnConfig.fill" />
    <loading class="rotate" v-show="buttonnStatus === 'loading'" :theme="buttonnConfig.theme" :size="buttonnConfig.size"
      :fill="buttonnConfig.fill" />
    <check-one v-show="buttonnStatus === 'success'" :theme="buttonnConfig.theme" :size="buttonnConfig.size"
      :fill="buttonnConfig.fill" />
    <close-one v-show="buttonnStatus === 'error'" :theme="buttonnConfig.theme" :size="buttonnConfig.size"
      :fill="buttonnConfig.fill" />
    <span class="text-xs ml-0.5 text-gray-500 leading-none">{{
      buttonnStatusTips[buttonnStatus]
      }}</span>
  </div>
</template>
